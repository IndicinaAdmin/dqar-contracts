...
class MeasureSQLLoader:
    ...
    Artifacts are packaged under cdar_contracts/measures/sql/<MP>/<measure_id>.json
    ...
    def _resolve_path(self, measure_id: str, measurement_period: str) -> Path:
        ...
        base = resources.files("cdar_contracts.measures") / "sql" / measurement_period
    ...
    def list_available(self, measurement_period: str) -> list[str]:
        ...
        if self._artifact_root is not None:
            ...
            mp_dir = Path(str(
                resources.files("cdar_contracts.measures") / "sql" / measurement_period
            ))
