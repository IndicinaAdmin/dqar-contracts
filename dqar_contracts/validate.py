"""
dqar-contracts validate CLI.

Usage:
    python -m dqar_contracts.validate --schema egress_package --file ./path/to/manifest.json
    python -m dqar_contracts.validate --schema feed_manifest --file ./feed-manifest.json
    python -m dqar_contracts.validate --schema engagement --file ./config/engagements/client.json
    python -m dqar_contracts.validate --list-viewdefs   # check all ViewDefs have getResourceKey()
...
def main():
    parser = argparse.ArgumentParser(description="DQAR contracts validator")
